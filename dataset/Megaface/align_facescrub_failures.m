...
if ~exist('image_list','var')
    ...
    while ~feof(actor_file)
        ...
        slash_pos = strfind(splited{4},'/');
        slash_pos = slash_pos(end);
        dot_pos = strfind(splited{4}(slash_pos:end),'.');
        if isempty(dot_pos)
            if exist([folder splited{1} '/' splited{1} '_' splited{3}],'file')
                movefile([folder splited{1} '/' splited{1} '_' splited{3}], [folder splited{1} '/' splited{1} '_' splited{3} '.jpg']);
            end;
            suffix = '.jpg';
        else
            dot_pos = dot_pos(end);
...
for tl = 1:length(test_list)
    ...
    for i=1:length(image_list)
        if strcmp(image_list{i}, test_list{tl}) 
            image_id = i;
            break;
        end;
    end;
...
%         imwrite(cropImg, target_filename);
        ...
        if isempty(result.score)
            result = MatMTCNN('force_detect',img,boxes(image_id,:));
            default_face = 1;
            facial5points = [result.points(default_face,1:2:9);result.points(default_face,2:2:10)];
            Tfm =  cp2tform(facial5points', coord5points', 'similarity');
            cropImg = imtransform(img, Tfm, 'XData', [1 imgSize(2)], 'YData', [1 imgSize(1)], 'Size', imgSize);
%             imwrite(cropImg, target_filename);
        ...
        if ~isempty(result.bounding_box)
            ...
            fprintf('%d/%d, %s detected=%d\n',image_id,image_list_len,target_filename,size(result.bounding_box,1));
        end;
    end;
